...
**Reviewed at commit:** `ce7cf2a` (`fix(inventory): prevent stock desynchronization`) — the last **backend** commit; see the entry below for frontend-only work since then.
**Last targeted update:** 2026-09-18 — `/admin/orders` Management Console module implemented (Orders-only; list + detail + permission-gated status-transition actions), consuming the existing `/api/staff/orders/` API with zero backend changes. See [Review History](#21-review-history).
...
**Frontend routes:** `/admin`, `/admin/login`, `/admin/profile`, and list/detail(/new) families for `shops`, `sellers`, `users`, `roles`, `products` (no `new`), `categories`, `customers`, **`orders`** (list + detail, no `new` — orders are never created by staff), plus a catch-all `/admin/[...slug]` that renders an "Awaiting Implementation" placeholder for the remaining nav entries with no page yet (`/admin/payments`, `/admin/audit-logs`).
...
- **Admin console modules not built**: `/admin/payments`, `/admin/audit-logs` appear in navigation but render the "Awaiting Implementation" placeholder (both APIs already exist backend-side). `/admin/orders` was built 2026-09-18 — see [Review History](#21-review-history).
...
**Last completed change:** `/admin/orders` Management Console module (Phase 1J) — Next.js Orders list + detail with permission-gated status-transition actions, built entirely on the existing `/api/staff/orders/` API. Zero backend changes. A feature, not a fix.
...
**Last completed feature:** `/admin/orders` Management Console module (this entry).
**Preceding feature:** Product Reviews & Ratings (`c4fb3b8`).

**Working tree:** backend clean at `ce7cf2a`; this update adds frontend-only changes (`frontend/src/lib/admin-api.ts`, `frontend/src/app/admin/orders/`) plus this file.

**What is implemented:** the full commerce chain (catalog → cart → checkout → orders → inventory → payments/refunds), the seller panel, the management console (now including Orders), RBAC governance, points/wallet, and reviews.

**Next planned phase — evidence-based, set by the 2026-09-18 architecture audit, and updated now that its recommendation is done.** No file in this repo defines a formal "Phase 1J" — the label is this file's own continuation of the informal Phase-1[A-I] naming already used for the admin console build-out (`task/task1.md`–`task17.md` cover unrelated backend tasks 1–17, and `forAdminPanel/*.md` is a separate, already-complete effort scoped to the **Django** admin at `/admin/`). "Phase 1J" is used here only as that continuation label, applied to the Orders module just shipped.
With Orders done, the same audit's reasoning carries forward unchanged to the two modules deliberately left out of this task: **`/admin/payments`** and **`/admin/audit-logs`** are both still "Missing" (placeholder-only) and both still fully backend-ready (`GET /api/staff/payments/...` + `/verify/` + `/refund/`; `GET /api/admin/audit-logs/`, read-only), with RBAC permissions already seeded (`payments.verify/refund`; `audit.view`/`audit.admin.view`/`users.admin.view`/`roles.admin.view`). Either is a reasonable next module; Payments is the more natural follow-on since it completes the same order-fulfillment surface Orders just opened, while Audit Logs is a simpler read-only list. Neither was started here, matching the project's one-module-per-task precedent.
