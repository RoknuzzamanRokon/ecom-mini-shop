...
**Reviewed at commit:** `382dd39` (`fix(security): harden seller and shop admin actions`)
**Last targeted update:** 2026-09-18 — Stock Desynchronization #2 audit (fixed). `ProductService.update_product` now routes `stock` through `InventoryService` under row lock instead of writing `Product.stock` directly. See [Review History](#21-review-history).
...
**Last completed change:** Stock Desynchronization #2 fix — `ProductService.update_product` no longer writes `Product.stock` directly; it routes through `InventoryService.adjust_stock` under row lock. A targeted data-integrity fix, not a feature. Audited and confirmed there was no overselling/security exposure.
