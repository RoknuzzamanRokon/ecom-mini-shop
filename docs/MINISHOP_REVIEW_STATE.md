...
**Reviewed at commit:** `ce7cf2a` (`fix(inventory): prevent stock desynchronization`) — the last commit before this task; the current HEAD adds one focused backend fix (below).
**Last targeted update:** 2026-09-18 — Task #6 implemented: seller-lifecycle API mutations (`sellers/views.py` approve/reject/suspend/reactivate) now write an `AuditLog` via `AuditService.log`, matching the `ADMIN_SELLER_<ACTION>` convention already used by `shop/admin_views.py`'s `AdminSellerStatusAPIView` and by the Django-admin seller actions. Backend-only; no frontend, RBAC, migration, or model changes. See [Review History](#21-review-history).
...
**Next planned phase — re-evaluated 2026-09-18 (Task #6 complete).** Known Issue #6 (seller-lifecycle API mutations unaudited) — the single task selected by the preceding post-Payments audit — is now fixed; see §16 for the implementation and §21 for the full record. With #6 closed, `/admin/audit-logs` (the sole remaining "Missing" console module) is again the most-discussed candidate, but per this file's own recurring caveat, a future review should re-scan §16 once more before committing to it, in case something else has become more urgent in the meantime — the same check that redirected the prior review away from Audit Logs and onto #6.
...
**Immediate follow-ups:** the legacy template checkout still bypasses `OrderService` (#1b, confirmed to be a correctness gap rather than a security one) — decide whether to route it through the service or retire the template flow; decide intent on seller self-registration (#5); the one failing test (#14) is a trivial assertion fix. The legacy order-success IDOR (#1), the whole unguarded-admin-action class (#15, #16), the `Product.stock` / `ProductInventory` desync on seller product update (#2), and the seller-lifecycle API audit gap (#6) are all done. Four new low-severity architectural findings from the Payments audit (#18–#21: unfiltered `metadata` passthrough, refund-serializer missing an upper bound, an orphaned `CanCreatePayment` permission class, and an unfiltered `StaffRefundListAPIView`) remain open but none are urgent — see §16.
